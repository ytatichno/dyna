...
struct PragmaActualCall {
  ...
  PragmaActualCall(addr_t baseAddr,
                   ActualCallString *cs,
                   std::vector<uint32_t> &&args)
      : baseAddr(baseAddr), cs(cs), args(args) {
  }
  ~PragmaActualCall() {}
...
public: // methods
  ...
  // Добавляет информацию о контекстной строке в хранилище информации о
  // контекстных строках
  void AddContextString(CSHandle *keyAddress, char *str);
  // Регистрирует переменную в текущей ПЕ
  void RegVariable(CSHandle staticContextHandle, void *pAddr,
                   long *arrSize = NULL);
  void RegArray(CSHandle context_string, void *addr, size_t arr_size);
  // Регистрируем доступ к памяти
  void RegAccess(dyna::AccessType accType, CSHandle staticContextHandle,
                 void *pAddr, CSHandle var_name, void *pBase,
                 bool arrAcc = false);
  // Регистрируем цикл
  void RegLoop(CSHandle staticContextHandle, long *init, long *last,
               long *step);
  ...
  void RegActualParameter(CSHandle staticContextHandle, int pos,
                          CSHandle var_name);
  // Регистрирует формальный параметр функции
  void RegFormalParameter(CSHandle staticContextHandle, int pos, void *pAddr,
                          long *arrSize = NULL);
  // Регистрирует операцию pragma dvm actual
  void RegPragmaActual(CSHandle staticContextHandle, addr_t baseAddr,
                       std::vector<uint32_t> args);
  // Регистрирует операцию pragma dvm get_actual undone
  void RegPragmaGetActual(CSHandle staticContextHandle, addr_t baseAddr,
                          std::vector<uint32_t> args);
...
private: // variables
  ContextStringsStore
      *m_contextStringsStore; // хранилище информации из контестных строк.
  dyna::ContextStack m_contexts; // стек контекстов отражающий текущее состояние
                                 // программы во время выполнения.
  dyna::AnalysisStorage
      m_anstorage; // глобальное хранилище результатов анализа.
  dyna::RegionActualMap
      m_actualityStorage; // hide it behind conditional compilation
...
private: // methods
  ...
  InterfaceCallsInfo &m_calls_info(const std::string &name) {
    ...
    if (it == m_calls_info_map.end()) {
      auto ret =
          m_calls_info_map.insert(std::make_pair(name, std::make_pair(0.0, 0)));
  ...
  /// @deprecated
  inline void m_actual_state_trans(addr_t addr,
                                   ActualCallString *contextString);
  ...
  inline SrcRefString * m_redundant_copy_to_gpu(addr_t addr, ActualCallString *cs);
  inline SrcRefString* m_redundant_copy_from_gpu(addr_t addr, GetActualCallString *cs);
  inline unsigned m_addr_to_elem(addr_t addr, addr_t base, unsigned size);
  // inline int32_t m_get_type_size(int32_t vtype) { return
  // m_types_table[vtype]; }
};
/*********************************************************************************************/
